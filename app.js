...
with check ((select auth.uid()) = user_id);`;
  ...
  function checkoutUrl() {
    return APP_CONFIG.checkoutUrl || "./buy.html";
  }
  ...
  function analyzeRls(input) {
    ...
    const warnings = buildStaticWarnings(input, rlsEnabled, matchingPolicies, policies, matchingGrants, matchingIndexes);
  ...
  function buildStaticWarnings(input, rlsEnabled, matchingPolicies, allPolicies, matchingGrants = [], matchingIndexes = []) {
    const warnings = [];
...
rollback;`;
  ...
  function authUidComparedColumns(policy) {
    ...
    while ((match = left.exec(expression))) {
      columns.push(unquoteSqlName(match[1]).toLowerCase());
    }
    while ((match = right.exec(expression))) {
      columns.push(unquoteSqlName(match[1]).toLowerCase());
    }
  ...
  class ExpressionParser {
    ...
    parseComparison() {
      ...
      left = this.parseJsonPostfix(left);
      const token = this.peek();
      ...
      if (this.consume("in")) {
        ...
        while (!this.consume(")")) {
          ...
          value = this.parseJsonPostfix(value);
          values.push(value);
          this.consume(",");
      ...
      if (this.consume("like")) {
        ...
        right = this.parseJsonPostfix(right);
