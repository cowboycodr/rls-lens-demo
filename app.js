...
with check (
  ...
  const SIGNUP_PROFILE_SQL = `create table public.profiles (
...
with check (auth.uid() = id);`;
  ...
  const PUBLIC_FEEDBACK_SQL = `create table public.feedback (
  id bigint generated always as identity primary key,
...
with check (true);`;
  ...
  const PUBLIC_FEEDBACK_NEW_ROW = {
    message: "The feedback form fails in production.",
    contact_email: "visitor@example.test",
    source: "web",
    status: "new",
  };

  const BROAD_AUTHENTICATED_SQL = `create table public.orders (
  id bigint generated always as identity primary key,
...
create policy "orders authenticated can do everything"
on public.orders
for all
to authenticated
using (true)
with check (true);`;
  ...
  const BROAD_AUTHENTICATED_ROWS = [
    {
      id: 1,
  ...
  const BROAD_AUTHENTICATED_NEW_ROW = {
    status: "submitted",
    total_cents: 100,
  };
  ...
  const DEEP_LINK_SCENARIOS = {
    ...
    "signup-profile-anon-insert": {
      status: "Scenario loaded: signUp returned a user, but the profile insert is still running as anon.",
      tab: "fixes",
      state: {
        ...DEFAULT_STATE,
        operation: "insert",
        ...
        sql: SIGNUP_PROFILE_SQL,
        rows: [],
        newRow: SIGNUP_PROFILE_NEW_ROW,
        jwt: { role: "anon" },
      },
    },
    "anonymous-feedback-broad-insert": {
      status: "Scenario loaded: anonymous feedback INSERT is allowed, but the policy accepts any row shape.",
      tab: "fixes",
      state: {
        ...DEFAULT_STATE,
        operation: "insert",
        ...
        sql: PUBLIC_FEEDBACK_SQL,
        rows: [],
        newRow: PUBLIC_FEEDBACK_NEW_ROW,
        jwt: { role: "anon" },
      },
    },
    "authenticated-orders-broad-access": {
      status: "Scenario loaded: authenticated users can update every order because both RLS phases are true.",
      tab: "matrix",
      state: {
        ...DEFAULT_STATE,
        operation: "update",
        table: "public.orders",
        sql: BROAD_AUTHENTICATED_SQL,
        rows: BROAD_AUTHENTICATED_ROWS,
        newRow: BROAD_AUTHENTICATED_NEW_ROW,
      },
    },
  };
  ...
  function buildStaticWarnings(input, rlsEnabled, matchingPolicies, allPolicies, matchingGrants = [], matchingIndexes = [], tables = new Map()) {
    ...
    warnings.push(...buildAnonInsertAuthUidWarnings(input, matchingPolicies));
    warnings.push(...buildBroadAnonInsertWarnings(input, matchingPolicies));
    warnings.push(...buildBroadRolePolicyWarnings(input, matchingPolicies));
  ...
  function buildAnonInsertAuthUidWarnings(input, matchingPolicies) {
    if (input.operation !== "insert" || input.role !== "anon") return [];
    const authenticatedOwnerPolicies = matchingPolicies.filter((policy) => {
      if (!(policy.command === "all" || policy.command === "insert")) return false;
      if (!policy.roles.includes("authenticated")) return false;
      return /\bauth\.uid\s*\(/i.test(policy.withCheck || policy.using || "");
    });
    if (!authenticatedOwnerPolicies.length) return [];

    return [
      {
        level: "danger",
        title: "INSERT is running as anon while the policy expects auth.uid()",
        body:
          "If this happens right after supabase.auth.signUp(), email confirmation can mean authData.user exists while authData.session is still null. Create the profile from an auth.users trigger or wait for an authenticated session before inserting.",
      },
    ];
  }

  function buildBroadAnonInsertWarnings(input, matchingPolicies) {
    if (input.operation !== "insert" || input.role !== "anon") return [];
    return matchingPolicies
      .filter((policy) => policyApplies(policy, "insert", "anon"))
      .filter((policy) => isBroadAllowAllExpression(policy.withCheck || policy.using || ""))
      .map((policy) => ({
        level: "danger",
        title: `Anonymous INSERT policy "${policy.name}" accepts any row`,
        body:
          "Public forms sometimes need anon INSERT, but WITH CHECK (true) lets any anonymous request choose every writable column. Keep reads/updates blocked and narrow the insert row with server validation, fixed defaults, or explicit checks such as source/status constraints.",
      }));
  }

  function buildBroadRolePolicyWarnings(input, matchingPolicies) {
    if (input.role === "service_role") return [];
    if (input.operation === "insert" && input.role === "anon") return [];

    return matchingPolicies
      .filter((policy) => policyApplies(policy, input.operation, input.role))
      .flatMap((policy) =>
        policyPhases(input.operation)
          .map((phase) => ({
            policy,
            phase,
            expression: expressionForPhase(policy, phase),
          }))
          .filter((item) => isBroadAllowAllExpression(item.expression || ""))
          .map((item) => ({
            level: "danger",
            title: `Policy "${item.policy.name}" allows ${input.operation.toUpperCase()} with ${item.phase.name} (true)`,
            body: `${input.role} requests can pass the ${item.phase.name} phase for ${input.table} without an owner, tenant, status, or claim check. If app-layer authorization is intended, move the write through trusted server code or replace this with auth.uid()/claim-based predicates and tests for other users.`,
          }))
      );
  }

  function isBroadAllowAllExpression(expression) {
    const normalized = prepareExpression(expression || "")
      .replace(/\s+/g, " ")
      .trim();
    if (!normalized) return false;
    return /^(?:\(\s*)*true(?:\s*\))*$/i.test(normalized) || /^(?:\(\s*)*1\s*=\s*1(?:\s*\))*$/i.test(normalized);
  }
