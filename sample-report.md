...
- DANGER: New row omits auth.uid() owner column "user_id" - Insert policies that check auth.uid() against user_id need the inserted row or a database default to provide user_id before WITH CHECK runs.
